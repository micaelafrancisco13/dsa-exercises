...
public class Tree {
    ...
    private boolean isBinarySearchTree(Node currentRoot, double startingRange, double endingRange) {
        if (currentRoot == null || isLeaf(currentRoot)) return true;

        if (currentRoot.value < startingRange || currentRoot.value > endingRange) return false;
    ...
    public ArrayList<Node> traverseLevelOrder() {
        var allNodes = new ArrayList<Node>();

        System.out.println("height " + height());

        for (var i = 0; i <= height(); ++i)
            allNodes.addAll(findNodesAtGivenDistance(i));
